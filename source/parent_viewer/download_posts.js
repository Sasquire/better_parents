...
BP.download_all = async function(page_text){
	...
	for(let child of post.children){
		await BP.download_post_graph(child);
	}
	if(post.parent_id){
		await BP.download_post_graph(post.parent_id);
	}
	BP.each_ended(page_id);
	return;
};

BP.download_post_graph = async function(start_id){
	if(BP.posts[start_id]){ return; }
	await BP.add_post(start_id);

	const cur_post = BP.posts[start_id];
	for(let child of cur_post.children){
		await BP.download_post_graph(child);
	}
	if(cur_post.parent_id){
		await BP.download_post_graph(cur_post.parent_id);
	}
	return;
};
