...
Object.defineProperties(BP.posts, {
	// Non enumerable and non-writable by default
	get_nodes: {
		value: function() {
			return Object.values(BP.posts).map((post_obj) => ({
				id: post_obj.post_id,
				img: post_obj.source
			}));
		}
	...
	get_links: {
		value: function() {
			return Object.values(BP.posts).map((post_obj) =>
				post_obj.children.map(child_id => ({
					source: child_id,
					target: post_obj.post_id
				})).concat({
					source: post_obj.post_id,
					target: post_obj.parent_id
				})
			)
			.reduce((acc, link) => acc.concat(...link), [])
			.filter(e => e.target != null)
			.filter((e, idx, arr) => arr.findIndex(val => val.target == e.target && val.source == e.source) === idx);
		}
	}
});
